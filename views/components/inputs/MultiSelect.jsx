...
function MultiSelect({ name, value, label, placeholder, className, onChange, options, errorMessage }) {
    ...
    function onClick(item, e){
        ...
        let index = updateState.findIndex(v=>v.name === item.name);
        if(index === -1){
    ...
    function deleteSelectedInput(item){
        let newState = value && value.filter(v=>v.name !== item.name)
    ...
    return (
        <div className={["multi-select", className].join(" ")} >
            ...
            <div className="w-full">
                ...
                <div className="input shrink w-full text-gray-300 relative flex items-center"
